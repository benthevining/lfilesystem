...
add_test (NAME "${base_name}.install"
		  COMMAND "${CMAKE_COMMAND}" 
		  			--install "${lfilesystem_BINARY_DIR}"
		  			--config $<CONFIG>
		  			--prefix "${install_dir}"
...
set_tests_properties ("${base_name}.configure" PROPERTIES 
	FIXTURES_SETUP LimesFilesFindPackageConfigure
...
set_tests_properties ("${base_name}.build" PROPERTIES FIXTURES_REQUIRED LimesFilesFindPackageConfigure)

add_test (NAME "${base_name}.clean"
		  COMMAND "${CMAKE_COMMAND}" -E rm -rf "${build_dir}" "${install_dir}")

set_tests_properties ("${base_name}.clean" PROPERTIES 
	FIXTURES_CLEANUP "LimesFilesFindPackageConfigure;LimesFilesFindPackageInstall")
